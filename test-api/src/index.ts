...
async function handleErrors(req: Request, func: () => Promise<Response>) {
	try {
		return await func();
	} catch (err) {
...
async function handleApiRequest(path: string[], req: Request, env: Env) {
	...
	switch (path[0]) {
		case "room": {
			...
			return roomObject.fetch(newUrl, req);
		}
